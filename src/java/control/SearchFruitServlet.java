...
public class SearchFruitServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("searchfruit") == "") {
            response.sendRedirect("FruitsServlet");
        } else {
            String name_Search = request.getParameter("searchfruit");
            FruitDAO fruitDAO = new FruitDAO();
            List<Fruit> fruits_Search = fruitDAO.GetListSearchFruit(name_Search);
            if(fruits_Search.isEmpty()){
                request.setAttribute("messageNotFound", "Can't not find for\""+name_Search+"\".");
            }
