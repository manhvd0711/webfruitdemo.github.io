...
public class ViewCart extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        {
        request.getRequestDispatcher("CheckLoginView.jsp").forward(request, response);
        }
        else if(session.getAttribute("userTarget") != null)
        {
            ...
            orders= dao.getListOrder(userid);
            fruits= dao.getListFruit(userid);
            request.setAttribute("fruitlists", fruits);
            request.setAttribute("orderlists", orders);
//            PrintWriter out= response.getWriter();
//            out.println("<h1>"+fruits.get(1).getProductid()+"<h1>");
